...
<form style={{display: 'flex', flexDirection: 'column'}}>
  ...
  value="Calculate"
  onClick={async (e) => {
    const _variables = variables.filter((v) =>
      selectedColIds.includes(getColNameFromVarName(v.name))
    );
    const results: any[] = new Array(_variables.length);
    const promises: any[] = new Array(_variables.length);
    var f: string[] = [];

    if (selMean) f.push("mean");
    if (selMedian) f.push("median");
    if (selMode) f.push("mode");
    if (selStd) f.push("std");
    if (selMin) f.push("min");
    if (selMax) f.push("max");
    if (selUnique) f.push("unique");
    ...
    _variables.forEach((v, idx) => {
      ...
      v.values.forEach((v2) =>
        data.append("data[]", String(v2 === undefined ? null : v2))
      );
    ...
    try {
      ...
      if (f.includes("unique")) {
        results
          .filter((r) => r.var.type === VariableType.NUMERICAL)
          .forEach((r) => (r.data["unique"] = r.data["unique"].length));
      }
      setColumns2([
        {
          columnId: "Statistic",
      ...
      showWarning(`An error occurred while processing the data: ${errorMessage}`);
    }
  }}
/>
  </form>
