...
export function FileImportModal({isFileImportModalOpen, setFileImportModalOpen, updateSpreadsheet, csvOutput} : FileImportModalProps) {
    ...
    return <div>
        ...
        <br></br>
        <input style={{margin: 5}} type="button" value="Next" className="bu-button bu-is-light bu-is-normal" onClick={(e) => {
            if (selectedSeparator === selectedDecimalSeparator)
            {
              ...
              {
                _variables.push({name: csvHeader[i].replaceAll("\n","").replaceAll("\r",""), type: VariableType.CATEGORICAL, values: []});
                isNumber.push(true);
              }
